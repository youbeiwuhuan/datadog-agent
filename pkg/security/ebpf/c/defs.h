...
#define SYSCALL_ABI_HOOKx(x,abi,ABI,type,TYPE,prefix,syscall,...) \
    long __attribute__((always_inline)) type##__##sys##syscall(__JOIN(x,__SC_DECL,__VA_ARGS__)); \
    SEC(#type "/" ABI##_SYSCALL_PREFIX #prefix SYSCALL_PREFIX #syscall) \
    long type##__ ##abi##_##prefix ##sys##syscall(struct pt_regs *ctx) { \
        SYSCALL_##TYPE##_PROLOG(x,__SC_##ABI##_PARAM,syscall,__VA_ARGS__) \
        return type##__sys##syscall(__JOIN(x,__SC_PASS,__VA_ARGS__)); \
    }

#define SYSCALL_HOOK_COMMON(x,type,syscall,...) long __attribute__((always_inline)) type##__sys##syscall(__JOIN(x,__SC_DECL,__VA_ARGS__))

#if USE_SYSCALL_WRAPPER == 1
  #define X64_SYSCALL_PREFIX "__x64_"
  #define IA32_SYSCALL_PREFIX "__ia32_"
  #define SYSCALL_PREFIX "sys"
  #define COMPAT_SYSCALL_PREFIX "compat_sys_"
  #define __SC_X64_PARAM(n, t, a) t a; bpf_probe_read(&a, sizeof(t), (void*) &X64_PT_REGS_PARM##n(ctx));
  #define __SC_IA32_PARAM(n, t, a) t a; bpf_probe_read(&a, sizeof(t), (void*) &IA32_PT_REGS_PARM##n(ctx));
  ...
  #define SYSCALL_HOOKx(x,type,TYPE,prefix,name,...) \
    SYSCALL_ABI_HOOKx(x,ia32,IA32,type,TYPE,prefix,name,__VA_ARGS__) \
    SYSCALL_ABI_HOOKx(x,x64,X64,type,TYPE,,name,__VA_ARGS__) \
    SYSCALL_HOOK_COMMON(x,type,name,__VA_ARGS__)
#else
  #define X64_SYSCALL_PREFIX ""
  #define IA32_SYSCALL_PREFIX ""
  #define SYSCALL_PREFIX "SyS"
  #define COMPAT_SYSCALL_PREFIX "compat_SyS_"
  #define __SC_X64_PARAM(n, t, a) t a = (t) X64_PT_REGS_PARM##n(ctx);
  #define __SC_IA32_PARAM(n, t, a) t a = (t) IA32_PT_REGS_PARM##n(ctx);
  #define SYSCALL_KPROBE_PROLOG(x,m,syscall,...) \
    __MAP(x,m,__VA_ARGS__)
  #define SYSCALL_KRETPROBE_PROLOG(...)
  #define SYSCALL_HOOKx(x,type,TYPE,prefix,name,...) \
